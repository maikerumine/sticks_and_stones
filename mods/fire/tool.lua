...
fire.apagar_tocha = function(player)
	local inv = player:get_inventory()
	-- Verifica cada um dos itens
	for _,list in pairs(inv:get_lists()) do
		for i,item in ipairs(list) do
			-- Troca pela tocha apagada
			if item:get_name() == "fire:torch_tool_on" then
				item:set_name("fire:torch_tool")
...
local desgaste_loop = (65535/fire.tempo_tocha)*2
fire.loop_tocha = function(name)
	...
	if itemstack:get_name() ~= "fire:torch_tool_on" then
		-- Encerra loop
		fire.apagar_tocha(player)
		fire.em_loop[name] = nil
		return
	end
	-- Adiciona desgaste
	...
	minetest.after(2, fire.loop_tocha, name)
end


-- Acender tocha
fire.acender_tocha = function(itemstack, player)
	local name = player:get_player_name()
	itemstack:set_name("fire:torch_tool_on")
	itemstack:add_wear(100)
	
	-- Verifica se ja esta acessa (evitar loop duplo)
	if not fire.em_loop[name] then 
		fire.em_loop[name] = {lpos=fire.get_lpos(player)}
		-- Inicia loop de tocha (atraso para dar tempo de atualizar item no inventario)
		minetest.after(0.3, fire.loop_tocha, name)
		-- Inicia loop de luz
		fire.loop_luz(name)
	end
...
minetest.register_tool("fire:torch_tool", {
	description = "Tocha",
	inventory_image = "hardtorch_torch_tool_off.png",
	groups = {not_in_creative_inventory = 1},
	
	on_use = function(itemstack, user, pointed_thing)
		return fire.acender_tocha(itemstack, user)
	end,
...
-- Versao acessa
minetest.register_tool("fire:torch_tool_on", {
	description = "Tocha",
	inventory_image = "default_torch_on_floor.png",
	groups = {not_in_creative_inventory = 1},
	
	on_use = function(itemstack, user, pointed_thing)
		itemstack:set_name("fire:torch_tool")
		return itemstack
	end,
	...
	-- Ao colocar funciona como tocha normal apenas repassando o desgaste
	on_place = minetest.registered_tools["fire:torch_tool"].on_place
...
minetest.register_on_joinplayer(function(player)
	...
	if inv:contains_item("main", "fire:torch_tool_on") 
		or inv:contains_item("craft", "fire:torch_tool_on") 
	then 
		fire.apagar_tocha(player)
	end
