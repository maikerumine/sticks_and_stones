...
fire.interval_low = 17
fire.interval_high = 503
fire.range_h = 100
fire.range_v = 50
fire.size = 100
...
local function choose_pos(pos)
	if not pos then
		...
		pos.x = math.floor(pos.x - (fire.range_h / 2) + rng:next(1, fire.range_h))
		pos.y = pos.y + (fire.range_v / 2)
		pos.z = math.floor(pos.z - (fire.range_h / 2) + rng:next(1, fire.range_h))
	end

	local b, pos2 = minetest.line_of_sight(pos, {x = pos.x, y = pos.y - fire.range_v, z = pos.z}, 1)
...
fire.strike = function(pos)
	if fire.auto then
		minetest.after(rng:next(fire.interval_low, fire.interval_high), fire.strike)
	end
	...
	minetest.add_particlespawner({
		...
		minpos = {x = pos2.x, y = pos2.y + (fire.size / 2) + 1/2, z = pos2.z },
		maxpos = {x = pos2.x, y = pos2.y + (fire.size / 2) + 1/2, z = pos2.z },
		minvel = {x = 0, y = 0, z = 0},
		maxvel = {x = 0, y = 0, z = 0},
		minacc = {x = 0, y = 0, z = 0},
		maxacc = {x = 0, y = 0, z = 0},
		minexptime = 0.2,
		maxexptime = 0.2,
		minsize = fire.size * 10,
		maxsize = fire.size * 10,
	...
	if minetest.get_item_group(minetest.get_node({x = pos2.x, y = pos2.y - 1, z = pos2.z}).name, "liquid") < 1 then
		if minetest.get_node(pos2).name == "air" then
			...
			if fire and rng:next(1,1000) == 1 then
				...
				minetest.set_node(pos2, {name = "fire:dying_flame"})
			end
		end
	end
...
minetest.register_node("fire:dying_flame", {
...
-- if other mods disable auto lightning during initialization, don't trigger the first fire.
minetest.after(5, function(dtime)
	if fire.auto then
		minetest.after(rng:next(fire.interval_low,
			fire.interval_high), fire.strike)
	end
end)
