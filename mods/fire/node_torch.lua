...
local on_dig = function(pos, node, digger)
	...
	local w_rest = (65535/fire.tempo_tocha)*t_rest
	local itemstack = {name="fire:torch_tool", count=1, wear=65535-w_rest, metadata=""}
...
local on_use = function(itemstack, player, pointed_thing)
	...
	itemstack:replace({name="fire:torch_tool_on", count=1, wear=0, metadata=""})
	...
	return fire.acender_tocha(itemstack, player)
end
...
local after_place_node = function(pos, placer, itemstack, pointed_thing)
	...
	minetest.get_node_timer(pos):start((fire.tempo_tocha/65535)*(65535-meta:get_int("wear")))
...
minetest.register_lbm({
	name = "fire:desgaste_tochas_antigas",
