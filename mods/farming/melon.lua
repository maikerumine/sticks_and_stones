...
minetest.register_node("farming:melon_seed", {
	...
	walkable = false,
	paramtype = "light",
	node_placement_prediction = "farming:melon_plant_1",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		farming.plant(pointed_thing.above, {name="farming:melon_plant_1"})
...
for stage = 1, 6 do
minetest.register_node("farming:melon_plant_" .. stage , {
...
minetest.register_node("farming:melon_plant_5_attached", {
	visual = "mesh",
...
minetest.register_node("farming:melon", {
	...
	on_dig = function(pos, node, digger)
		for face = 1, 4 do
			...
			if n.name == "farming:melon_plant_5_attached" then
				-- make sure it was actually attached to this stem
				if n.param2 == faces[face].o then
					minetest.swap_node(s, { name = "farming:melon_plant_4" })
				end
			end
		end
		...
		for i = 1,math.random(3 - (2 * (damage / 100)), 5 - (4 * (damage / 100))) do
			table.insert(drops, ('farming:melon_slice'))
		end
...
minetest.register_abm({
	nodenames = { "farming:melon_plant_1", "farming:melon_plant_2", "farming:melon_plant_3","farming:melon_plant_4" },
...
--
minetest.register_abm({
	nodenames = { "farming:melon_plant_5" },
	neighbors = { "group:soil" },
	interval = farming.settings.interval,
	chance = farming.settings.chance,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		for face = 1, 4 do
			...
			if minetest.get_node(t).name == "farming:melon" then
				return
			end
		end
		...
		if minetest.registered_nodes[n.name].drawtype == "plantlike" or
		   ...
		   n.name == "air" then
			minetest.swap_node(pos, {name = "farming:melon_plant_5_attached", param2 = faces[r].r})
			minetest.set_node(t, {name = "farming:melon", param2 = faces[r].m})
			local meta = minetest.get_meta(pos)
			local damage = meta:get_int("crops_damage")
...
--
minetest.register_abm({
	nodenames = { "farming:melon_plant_5_attached" },
	interval = farming.settings.interval,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		for face = 1, 4 do
		...
		minetest.swap_node(pos, {name = "farming:melon_plant_4" })
	end
})

farming.melon_die = function(pos)
	minetest.set_node(pos, { name = "farming:melon_plant_6" })
end

local properties = {
	die = farming.melon_die,
	waterstart = 20,
	wateruse = 1,
	night = 5,
	soak = 80,
...
farming.register({ name = "farming:melon_plant_1", properties = properties })
farming.register({ name = "farming:melon_plant_2", properties = properties })
farming.register({ name = "farming:melon_plant_3", properties = properties })
farming.register({ name = "farming:melon_plant_4", properties = properties })
farming.register({ name = "farming:melon_plant_5", properties = properties })
farming.register({ name = "farming:melon_plant_5_attached", properties = properties })
