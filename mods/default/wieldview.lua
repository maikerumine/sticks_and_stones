...
default.get_item_texture = function(self, item)
	...
	if item ~= "" then
		...
		if transform == 0 then
			transform = default.transform[item]
		end
		if transform then
...
default.update_wielded_item = function(self, player)
	if not player then
		return
	end
...
minetest.register_on_joinplayer(function(player)
	local name = player:get_player_name()
	default.wielded_item[name] = ""
	minetest.after(0, function(player)
		default:update_wielded_item(player)
	end, player)
end)
