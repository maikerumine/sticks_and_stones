...
minetest.register_node("default:cave_worms", {
	...
	for key, pos in pairs(stone_nodes) do
		...
		if spot ~= nil and math.random() < 0.001 and not near_surface(spot) then
		  ...
		  local posPos = { x=pos.x+6, y=pos.y+6, z=pos.z+6 }
			local worms = minetest.find_nodes_in_area(posNeg, posPos, "default:cave_worms")
...
function place_worms(pos)
	...
	for i, cpos in ipairs(axes) do
		if minetest.get_node(cpos).name == "default:stone" then
			...
			minetest.set_node(pos, { name = "default:cave_worms", param2 = facedir })
			return
		end
	end
end
...
minetest.register_node("default:stone_with_worms", {
	description = S("Glow Worms in Stone"),
	tiles = { "default_stone.png^worms.png" },
	is_ground_content = true,
	groups = { cracky = 1 },
	sounds = default.node_sound_stone_defaults(),
	drop = "default:stone_with_worms",
	paramtype = "light",
	light_source = 4,
})

minetest.register_abm({
	nodenames = { "default:stone_with_worms" },
	interval = 60.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
...
minetest.register_abm({
    ...
    local posPos = { x=pos.x+1, y=pos.y+1, z=pos.z+1 }
		local shrooms = minetest.find_nodes_in_area(posNeg, posPos, "default:shrooms")
...
function add_shrooms(pos)
	if minetest.find_node_near(pos, 2, "default:shrooms") == nil then
		for nx = -1, 1, 2 do
			for ny = -1, 1, 1 do
				for nz = -1, 1, 2 do
					...
					if minetest.get_node(tpos).name == "default:dirt_with_grass" and math.random() < 0.2 then
						...
						minetest.set_node(ppos, { name = "default:shrooms" })
					end
				end
			end
		end
	...
	if minetest.find_node_near(pos, 2, "default:shrooms") == nil then
		for nx = -1, 1, 2 do
			for ny = -1, 1, 1 do
				for nz = -1, 1, 2 do
					local tpos = { x=pos.x+nx, y=pos.y-1+ny, z=pos.z+nz }
					if minetest.get_node(tpos).name == "default:dirt_with_leaves_1" and math.random() < 0.2 then
						local ppos = { x=tpos.x, y=tpos.y+1, z=tpos.z }
						minetest.set_node(ppos, { name = "default:shrooms" })
					end
				end
...
minetest.register_node("default:fireflies", {
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		if minetest.get_timeofday() > 0.74 or minetest.get_timeofday() < 0.22 then
			if minetest.find_node_near(pos, 9, "default:fireflies") == nil then
				minetest.set_node(pos, {name = "default:fireflies"})
			end
		end
	end,
})
