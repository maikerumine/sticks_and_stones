...
DEFAULT_ADMIN_WEAPONS = false
DEFAULT_ENABLE_BLASTING = true
DEFAULT_ENABLE_CROSSBOW = false
DEFAULT_ENABLE_GUNS = true
DEFAULT_ENABLE_FLARES = false
DEFAULT_ENABLE_HOOK = false
DEFAULT_ENABLE_GRENADES = false
DEFAULT_ENABLE_ROCKETS = false
DEFAULT_ENABLE_TURRETS = false
DEFAULT_ENABLE_CRAFTING = true
DEFAULT_ENABLE_PARTICLE_FX = true
DEFAULT_ENABLE_PROTECTION = true
DEFAULT_EXPLOSION_TEXTURE = "shooter_hit.png"
DEFAULT_ALLOW_NODES = true
DEFAULT_ALLOW_ENTITIES = true
DEFAULT_ALLOW_PLAYERS = false
DEFAULT_OBJECT_RELOAD_TIME = 1
DEFAULT_OBJECT_UPDATE_TIME = 0.25
DEFAULT_ROUNDS_UPDATE_TIME = 8
...
local input = io.open(modpath.."/default.conf", "r")
if input then
	dofile(modpath.."/default.conf")
	input:close()
	input = nil
end
input = io.open(worldpath.."/default.conf", "r")
if input then
	dofile(worldpath.."/default.conf")
	input:close()
	input = nil
end
...
function default:play_node_sound(node, pos)
	...
	if item.groups then
		for k, v in pairs(def.groups) do
			...
			if level >= v then
				...
				default:play_node_sound(node, pos)
				if item.tiles then
...
function default:process_round(round)
	...
	local p1 = round.pos
	local v1 = round.ray
	for _,ref in ipairs(default.objects) do
...
default.registered_weapons = default.registered_weapons or {}
function default:register_weapon(name, def)
	default.registered_weapons[name] = def
				...
				if shots > 1 then
					...
					for i = 0, step * shots, step do
						minetest.after(i, function()
							default:fire_weapon(user, pointed_thing, def.spec)
						end)
					end
				else
					default:fire_weapon(user, pointed_thing, def.spec)
...
function default:fire_weapon(user, pointed_thing, def)
	if default.shots[def.name] then
		if default.time < default.shots[def.name] then
			return
		end
	end
	default.shots[def.name] = default.time + def.tool_caps.full_punch_interval
	...
	if pointed_thing.type == "node" and DEFAULT_ALLOW_NODES == true then
		local pos = minetest.get_pointed_thing_position(pointed_thing, false)
		default:punch_node(pos, def)
	elseif pointed_thing.type == "object" then
		local object = pointed_thing.ref
		if default:is_valid_object(object) == true then
			object:punch(user, nil, def.tool_caps, v1)
			local p2 = object:getpos()
			local pp = get_particle_pos(p1, v1, vector.distance(p1, p2))
			pp.y = pp.y + 1.75
			default:spawn_particles(pp, DEFAULT_EXPLOSION_TEXTURE)
		end
	else
		default:update_objects()
		table.insert(default.rounds, {
			name = def.name,
...
function default:load_objects()
	local objects = {}
	if DEFAULT_ALLOW_PLAYERS == true then
	...
	default.reload_time = default.time
	default.update_time = default.time
	default.objects = {}
	for _,v in ipairs(objects) do
		table.insert(default.objects, v)
	end
end

function default:update_objects()
	if default.time - default.reload_time > DEFAULT_OBJECT_RELOAD_TIME then
		default:load_objects()
	elseif default.time - default.update_time > DEFAULT_OBJECT_UPDATE_TIME then
		for i, ref in ipairs(default.objects) do
			if ref.object then
				local pos = ref.object:getpos()
				if pos then
					default.objects[i].pos = pos
				end
			else
				table.remove(default.objects, i)
			end
		end
		default.update_time = default.time
	end
...
function default:blast(pos, radius, fleshy, distance, user)
	if not user then
		return
	end
	...
	if DEFAULT_ALLOW_NODES and DEFAULT_ENABLE_BLASTING then
		if DEFAULT_ENABLE_PROTECTION then
	...
	for _,obj in ipairs(objects) do
		if (obj:is_player() and DEFAULT_ALLOW_PLAYERS == true) or
				(obj:get_luaentity() and DEFAULT_ALLOW_ENTITIES == true and
...
if not singleplayer and DEFAULT_ADMIN_WEAPONS then
	local timer = 0
		...
		for _,player in pairs(minetest.get_connected_players()) do
			if player:get_player_control().LMB then
				...
				if minetest.check_player_privs(name, {server=true}) then
					local spec = default.registered_weapons[player:get_wielded_item():get_name()]
					if spec then
						spec = spec.spec
						default.shots[name] = false
						spec.name = name
						default:fire_weapon(player, {}, spec)
						shooting = true
					end
				end
			end
...
default:register_weapon("default:rifle", {
...
minetest.register_craftitem("default:ammo", {
	description = "Ball Shot / Wad / and Powder",
	inventory_image = "shooter_ammo.png^[makealpha:255,255,255",
})

if DEFAULT_ENABLE_CRAFTING == true then

	minetest.register_craft({
		output = "default:rifle 1 65535",
		recipe = {
			{"", "default:flint", ""},
			{"", "default:iron_hardware", "default:steel_barrel_musket_reamed"},
	...
	minetest.register_craft({
		output = "default:rifle 1 65535",
		recipe = {
			{"default:rifle", "default:ammo"},
		},
	})
	
	minetest.register_craft({
		output = "default:ammo 12",
		recipe = {
			{"tnt:gunpowder", "default:plant_fiber", "default:steel_nugget"},
		},
	})
end
...
minetest.register_globalstep(function(dtime)
	default.time = default.time + dtime
	if default.time - rounds_update_time > DEFAULT_ROUNDS_UPDATE_TIME then
		for i, round in ipairs(default.rounds) do
			if default:process_round(round) or round.dist > round.def.range then
				table.remove(default.rounds, i)
			else
				local v = vector.multiply(round.ray, round.def.step)
				default.rounds[i].pos = vector.add(round.pos, v)
				default.rounds[i].dist = round.dist + round.def.step
			end
		end
		rounds_update_time = default.time
	end
	if default.time > 100000 then
		default.shots = {}
		rounds_update_time = 0
		default.reload_time = 0
		default.update_time = 0
		default.time = 0
	end
end)
...
Default configuration can be customised by adding a default.conf file to
the mod's main directory, see default.conf.example for more details.
