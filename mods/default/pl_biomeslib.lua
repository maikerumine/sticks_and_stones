...
default.blocklist_aircheck = {}
default.blocklist_no_aircheck = {}

default.surface_nodes_aircheck = {}
default.surface_nodes_no_aircheck = {}

default.surfaceslist_aircheck = {}
default.surfaceslist_no_aircheck = {}

default.actioncount_aircheck = {}
default.actioncount_no_aircheck = {}

default.actionslist_aircheck = {}
default.actionslist_no_aircheck = {}

default.modpath = minetest.get_modpath("default")

default.total_no_aircheck_calls = 0
...
default.plantlife_seed_diff = 329	-- needs to be global so other mods can see it
...
default.perlin_temperature = PerlinNoise(temperature_seeddiff, temperature_octaves, temperature_persistence, temperature_scale)
default.perlin_humidity = PerlinNoise(humidity_seeddiff, humidity_octaves, humidity_persistence, humidity_scale)

-- Local functions

function default:is_node_loaded(node_pos)
...
function default:set_defaults(biome)
...
function default:register_generate_plant(biomedef, nodes_or_function_or_model)
	...
	if type(nodes_or_function_or_model) == "string"
	  ...
	  and not minetest.registered_nodes[nodes_or_function_or_model] then
		default:dbg("Warning: Ignored registration for undefined spawn node: "..dump(nodes_or_function_or_model))
		return
	end

	if type(nodes_or_function_or_model) == "string"
	  and not string.find(nodes_or_function_or_model, ":") then
		default:dbg("Warning: Registered function call using deprecated string method: "..dump(nodes_or_function_or_model))
	end

	if biomedef.check_air == false then 
		default:dbg("Register no-air-check mapgen hook: "..dump(nodes_or_function_or_model))
		default.actionslist_no_aircheck[#default.actionslist_no_aircheck + 1] = { biomedef, nodes_or_function_or_model }
		local s = biomedef.surface
		if type(s) == "string" then
			if s and (string.find(s, "^group:") or minetest.registered_nodes[s]) then
				if not search_table(default.surfaceslist_no_aircheck, s) then
					default.surfaceslist_no_aircheck[#default.surfaceslist_no_aircheck + 1] = s
				end
			else
				default:dbg("Warning: Ignored no-air-check registration for undefined surface node: "..dump(s))
			end
		else
		...
		default:dbg("Register with-air-checking mapgen hook: "..dump(nodes_or_function_or_model))
		default.actionslist_aircheck[#default.actionslist_aircheck + 1] = { biomedef, nodes_or_function_or_model }
		local s = biomedef.surface
		if type(s) == "string" then
			if s and (string.find(s, "^group:") or minetest.registered_nodes[s]) then
				if not search_table(default.surfaceslist_aircheck, s) then
					default.surfaceslist_aircheck[#default.surfaceslist_aircheck + 1] = s
				end
			else
				default:dbg("Warning: Ignored with-air-checking registration for undefined surface node: "..dump(s))
			end
		else
			for i = 1, #biomedef.surface do
				local s = biomedef.surface[i]
...
function default:populate_surfaces(biome, nodes_or_function_or_model, snodes, checkair)

	default:set_defaults(biome)
	...
	for i = 1, #snodes do
		...
		local noise2 = default.perlin_temperature:get2d({x=pos.x, y=pos.z})
		local noise3 = default.perlin_humidity:get2d({x=pos.x+150, y=pos.z+50})
				...
				if not (biome.avoid_nodes and biome.avoid_radius and minetest.find_node_near(p_top, biome.avoid_radius + math.random(-1.5,2), biome.avoid_nodes)) then
					...
					if objtype == "table" then
						if nodes_or_function_or_model.axiom then
							default:generate_tree(pos, nodes_or_function_or_model)
							spawned = true
						else
							local fdir = nil
					...
					elseif objtype == "string" and pcall(loadstring(("return %s(...)"):
						...
						default:dbg("Warning: Ignored invalid definition for object "..dump(nodes_or_function_or_model).." that was pointed at {"..dump(pos).."}")
					end
				else
					tries = tries + 1
				end
...
function default:generate_block_with_air_checking()
	if #default.blocklist_aircheck > 0 then

		local minp =		default.blocklist_aircheck[1][1]
		local maxp =		default.blocklist_aircheck[1][2]
		...
		if not default.surface_nodes_aircheck.blockhash then

			if type(minetest.find_nodes_in_area_under_air) == "function" then -- use newer API call
				default.surface_nodes_aircheck.blockhash =
					minetest.find_nodes_in_area_under_air(minp, maxp, default.surfaceslist_aircheck)
			else
				local search_area = minetest.find_nodes_in_area(minp, maxp, default.surfaceslist_aircheck)
				...
				local pos = search_area[i]
					...
					if minetest.get_node(p_top).name == "air" then
						default.surface_nodes_aircheck.blockhash[#default.surface_nodes_aircheck.blockhash + 1] = pos
					end
				end
			end
			default.actioncount_aircheck.blockhash = 1

		else
			if default.actioncount_aircheck.blockhash <= #default.actionslist_aircheck then
				-- [1] is biome, [2] is node/function/model
				default:populate_surfaces(
					default.actionslist_aircheck[default.actioncount_aircheck.blockhash][1],
					default.actionslist_aircheck[default.actioncount_aircheck.blockhash][2],
					default.surface_nodes_aircheck.blockhash, true)
				default.actioncount_aircheck.blockhash = default.actioncount_aircheck.blockhash + 1
			else
				if default.surface_nodes_aircheck.blockhash then
					table.remove(default.blocklist_aircheck, 1)
					default.surface_nodes_aircheck.blockhash = nil
				end
			end
		end
	end
...
function default:generate_block_no_aircheck()
	if #default.blocklist_no_aircheck > 0 then

		local minp =		default.blocklist_no_aircheck[1][1]
		local maxp =		default.blocklist_no_aircheck[1][2]

		local blockhash =	minetest.hash_node_position(minp)

		if not default.surface_nodes_no_aircheck.blockhash then

			-- directly read the block to be searched into the chunk cache

			default.surface_nodes_no_aircheck.blockhash =
				minetest.find_nodes_in_area(minp, maxp, default.surfaceslist_no_aircheck)
			default.actioncount_no_aircheck.blockhash = 1

		else
			if default.actioncount_no_aircheck.blockhash <= #default.actionslist_no_aircheck then
				default:populate_surfaces(
					default.actionslist_no_aircheck[default.actioncount_no_aircheck.blockhash][1],
					default.actionslist_no_aircheck[default.actioncount_no_aircheck.blockhash][2],
					default.surface_nodes_no_aircheck.blockhash, false)
				default.actioncount_no_aircheck.blockhash = default.actioncount_no_aircheck.blockhash + 1
			else
				if default.surface_nodes_no_aircheck.blockhash then
					table.remove(default.blocklist_no_aircheck, 1)
					default.surface_nodes_no_aircheck.blockhash = nil
				end
			end
		end
	end
...
minetest.register_on_generated(function(minp, maxp, blockseed)
	default.blocklist_aircheck[#default.blocklist_aircheck + 1] = { minp, maxp }
end)

minetest.register_on_generated(function(minp, maxp, blockseed)
	default.blocklist_no_aircheck[#default.blocklist_no_aircheck + 1] = { minp, maxp }
end)
...
minetest.register_globalstep(function(dtime)
	if dtime < 0.2 and    -- don't attempt to populate if lag is already too high
	  (#default.blocklist_aircheck > 0 or #default.blocklist_no_aircheck > 0) then
		default.globalstep_start_time = minetest.get_us_time()
		default.globalstep_runtime = 0
		while (#default.blocklist_aircheck > 0 or #default.blocklist_no_aircheck > 0)
		  and default.globalstep_runtime < 200000 do  -- 0.2 seconds, in uS.
			if #default.blocklist_aircheck > 0 then
				default:generate_block_with_air_checking()
			end
			if #default.blocklist_no_aircheck > 0 then
				default:generate_block_no_aircheck()
			end
			default.globalstep_runtime = minetest.get_us_time() - default.globalstep_start_time
		end
	end
end)
...
minetest.register_on_shutdown(function()
	...
	print("(there are "..#default.blocklist_aircheck.." entries)...")
	while true do
		default:generate_block_with_air_checking(0.1)
		if #default.blocklist_aircheck == 0 then return end
	end
end)
...
minetest.register_on_shutdown(function()
	...
	print("(there are "..#default.blocklist_aircheck.." entries)...")
	while true do
		default:generate_block_no_aircheck(0.1)
		if #default.blocklist_no_aircheck == 0 then return end
	end
end)

-- The spawning ABM

function default:spawn_on_surfaces(sd,sp,sr,sc,ss,sa)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if noise1 > biome.plantlife_limit 
			  ...
			  and noise3 <= biome.humidity_min
			  and noise3 >= biome.humidity_max
			  and default:is_node_loaded(p_top) then
				...
				if not (biome.avoid_nodes and biome.avoid_radius and minetest.find_node_near(p_top, biome.avoid_radius + math.random(-1.5,2), biome.avoid_nodes))
				  ...
				  and pos.y <= biome.max_elevation
				  then
					local walldir = default:find_adjacent_wall(p_top, biome.verticals_list, biome.choose_random_wall)
						...
						if currentsurface ~= "default:water_source"
						  ...
						  then
							...
							elseif biome.spawn_on_side then
								local onside = default:find_open_side(pos)
								if onside then 
...
function default:grow_plants(opts)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if options.need_wall and options.verticals_list then
				walldir = default:find_adjacent_wall(p_top, options.verticals_list, options.choose_random_wall)
			end
			if (n_top.name == "air" or n_top.name == "default:snow")
			  and (not options.need_wall or (options.need_wall and walldir)) then
				...
				elseif options.grow_vertically and walldir then
					if default:search_downward(pos, options.height_limit, options.ground_nodes) then
				...
				elseif not options.grow_result and not options.grow_function then
					...
					default:replace_object(pos, options.grow_result, options.grow_function, options.facedir, options.seed_diff)
				end
			end
		end
	})
...
function default:replace_object(pos, replacement, grow_function, walldir, seeddiff)
	local growtype = type(grow_function)
	if growtype == "table" then
		minetest.remove_node(pos)
		default:grow_tree(pos, grow_function)
		return
...
function default:find_adjacent_wall(pos, verticals, randomflag)
...
function default:search_downward(pos, heightlimit, ground)
...
function default:find_open_side(pos)
...
function default:generate_tree(pos, nodes_or_function_or_model)
	minetest.spawn_tree(pos, nodes_or_function_or_model)
end

-- and this one's for the call used in the growing code

function default:grow_tree(pos, nodes_or_function_or_model)
	minetest.spawn_tree(pos, nodes_or_function_or_model)
end
...
if minetest.get_modpath("unified_inventory") or not minetest.setting_getbool("creative_mode") then
	default.expect_infinite_stacks = false
else
	default.expect_infinite_stacks = true
end

-- read a field from a node's definition

function default:get_nodedef_field(nodename, fieldname)
...
minetest.after(0, function()
	print("[Biome Lib] Registered a total of "..(#default.surfaceslist_aircheck)+(#default.surfaceslist_no_aircheck).." surface types to be evaluated, spread")
	print("[Biome Lib] across "..#default.actionslist_aircheck.." actions with air-checking and "..#default.actionslist_no_aircheck.." actions without.")
end)
