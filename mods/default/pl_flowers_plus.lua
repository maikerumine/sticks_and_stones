...
for i in ipairs(lilies_list) do
	...
	minetest.register_node(":flowers:waterlily"..deg1, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if default:get_nodedef_field(under_node.name, "buildable_to") then
				if under_node.name ~= "default:water_source" then
					place_pos = pt.under
				elseif top_node.name ~= "default:water_source" 
				       and default:get_nodedef_field(top_node.name, "buildable_to") then
					place_pos = top_pos
				else
					return
				end
			elseif default:get_nodedef_field(above_node.name, "buildable_to") then
				place_pos = pt.above
			end
			...
			local nodename = "default:cobble" -- if this block appears, something went....wrong :-)
				...
				if not default.expect_infinite_stacks then
					itemstack:take_item()
				end
				return itemstack
			end
...
default.grow_waterlily = function(pos)
...
default:register_generate_plant({
  ...
  default.grow_waterlily
)

default.grow_seaweed = function(pos)
	local right_here = {x=pos.x, y=pos.y+1, z=pos.z}
