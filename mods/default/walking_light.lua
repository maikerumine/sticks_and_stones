...
--minetest.register_alias("default:light", "default:lightz")
...
function update_light_player(pinfo)
	...
	for h,p in pairs(adds) do
		...
		if node == nil or (node ~= nil and node.name == "air") then
			minetest.env:add_node(p, {type="node",name="default:lights"})
		end
	end

	for h,p in pairs(removes) do
		local node = minetest.env:get_node_or_nil(p)
		if node ~= nil and node.name == "default:lights" then
...
minetest.register_node("default:lights", {
	--drawtype = "glasslike",
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		local meta = minetest.get_meta(pos)
			...
			hacky_swap_node(pos,"default:torch")
			return
	end,
})
