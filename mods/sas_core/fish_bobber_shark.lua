...
minetest.register_alias("default:bobber_shark_box", "sas_core:bobber_shark_box")
...
-- bobber shark
minetest.register_node("sas_core:bobber_shark_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
local FISHING_BOBBER_SHARK_ENTITY={
	...
	textures = {"sas_core:bobber_shark_box"},
	--			   {left ,bottom, front, right,  top ,  back}
...
--  DESTROY BOBBER WHEN PUNCHING IT
	on_punch = function (self, puncher, time_from_last_punch, tool_capabilities, dir)
		...
		if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, default_setting.func.S("You didn't catch anything."), false) end
		if not default_setting.is_creative_mode then
			...
			if inv:room_for_item("main", {name=self.bait, count=1, wear=0, metadata=""}) then
				...
				if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, default_setting.func.S("The bait is still there."), false) end
			end
		end
...
--	WHEN RIGHTCLICKING THE BOBBER THE FOLLOWING HAPPENS (CLICK AT THE RIGHT TIME WHILE HOLDING A FISHING POLE)
	on_rightclick = function (self, clicker)
		...
		if string.find(item_name, "sas_core:pole_") ~= nil then
			if player_name ~= self.owner then return end
			if self.prize ~= "" then
				if math.random(1, 100) <= default_setting.settings["escape_chance"] then
					if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, default_setting.func.S("Your fish escaped."), false) end -- fish escaped
				else
					local name = self.prize[1]..":"..self.prize[2]
					local desc = self.prize[4]
					if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, default_setting.func.S("You caught "..desc), false) end
					default_setting.func.add_to_trophies(clicker, self.prize[2], desc)
					local wear_value = default_setting.func.wear_value(self.prize[3])
		...
		elseif item_name == "sas_core:baitball_shark" then
			if not default_setting.is_creative_mode then
				inv:remove_item("main", "sas_core:baitball_shark")
			end
			self.baitball = 20
			--addparticle
...
-- AS SOON AS THE BOBBER IS PLACED IT WILL ACT LIKE
	on_step = function(self, dtime)
		...
		if not node or string.find(node.name, "water_source") == nil then
			if default_setting.settings["message"] == true then minetest.chat_send_player(self.owner, default_setting.func.S("Haha, Fishing is prohibited outside water!")) end
			self.object:remove()
			return
		end
		...
		if dist > default_setting.settings["bobber_view_range"] then
			minetest.sound_play("fishing_bobber1", {pos = self.object:getpos(),gain = 0.5,})
			self.object:remove()
			return
		end
		...
		if math.random(1, 100) > default_setting.baits[self.bait]["hungry"] + self.baitball then
			--Fish not hungry !(
		...
		if chance == 1 then
			if math.random(1, 100) <= default_setting.settings["treasure_chance"] and default_setting.settings["treasure_enable"] then
				self.prize = default_setting.prizes["treasure"][math.random(1,#default_setting.prizes["treasure"])]
			end
		elseif chance <= default_setting.settings["fish_chance"] then
			if self.water_type and self.water_type == "sea" then
				self.prize = default_setting.prizes["sea"]["big"][math.random(1,#default_setting.prizes["sea"]["big"])]
			else
				self.prize = default_setting.prizes["rivers"]["big"][math.random(1,#default_setting.prizes["rivers"]["big"])]
			end
		else
			if math.random(1, 100) <= 10 then
				self.prize = default_setting.func.get_loot()
			end
		end
...
minetest.register_entity("sas_core:bobber_shark_entity", FISHING_BOBBER_SHARK_ENTITY)
