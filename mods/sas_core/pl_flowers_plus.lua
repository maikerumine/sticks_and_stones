...
for i in ipairs(lilies_list) do
	local deg1 = ""
	local deg2 = ""
	local lily_groups = {snappy = 3,flammable=2,flower=1, plant_fiber = 1}

	if lilies_list[i][1] ~= nil then
		deg1 = "_"..lilies_list[i][1]
		deg2 = "_"..lilies_list[i][2]
		lily_groups = { snappy = 3,flammable=2,flower=1,plant_fiber = 1, not_in_creative_inventory=1 }
	end
	...
	minetest.register_node(":flowers:waterlily"..deg1, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if sas_core:get_nodedef_field(under_node.name, "buildable_to") then
				if under_node.name ~= "default:water_source" then
					place_pos = pt.under
				elseif top_node.name ~= "default:water_source" 
				       and sas_core:get_nodedef_field(top_node.name, "buildable_to") then
					place_pos = top_pos
				else
					return
				end
			elseif sas_core:get_nodedef_field(above_node.name, "buildable_to") then
				place_pos = pt.above
			end
			...
			local nodename = "default:cobble" -- if this block appears, something went....wrong :-)
				...
				if not sas_core.expect_infinite_stacks then
					itemstack:take_item()
				end
				return itemstack
			end
...
for i in ipairs(algae_list) do
	local num = ""
	local algae_groups = {snappy = 3,flammable=2,flower=1, plant_fiber = 1}

	if algae_list[i][1] ~= nil then
		num = "_"..algae_list[i][1]
		algae_groups = { snappy = 3,flammable=2,flower=1,plant_fiber = 1,  not_in_creative_inventory=1 }
	end
...
minetest.register_node(":flowers:sunflower", {
	...
	groups = { dig_immediate=3, flora=1, flammable=3, dig_by_water = 1 },
	sounds = default.node_sound_leaves_defaults(),
	selection_box = box,
	collision_box = box,
})
...
sas_core.grow_waterlily = function(pos)
...
sas_core:register_generate_plant({
  ...
  sas_core.grow_waterlily
)

sas_core.grow_seaweed = function(pos)
	local right_here = {x=pos.x, y=pos.y+1, z=pos.z}
...
sas_core:spawn_on_surfaces({
	...
	spawn_surfaces = {"sas_core:water_source"},
...
2012-07-31: Disable debug by sas_core.
