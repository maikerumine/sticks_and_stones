--Alias
minetest.register_alias("default:cave_worms", "sas_core:cave_worms")
minetest.register_alias("default:stone_with_worms", "sas_core:stone_with_worms")
minetest.register_alias("default:shrooms", "sas_core:shrooms")
minetest.register_alias("default:fireflies", "fireflies:firefly")
...
minetest.register_node("sas_core:cave_worms", {
	...
	for key, pos in pairs(stone_nodes) do
		...
		if spot ~= nil and math.random() < 0.001 and not near_surface(spot) then
		  ...
		  local posPos = { x=pos.x+6, y=pos.y+6, z=pos.z+6 }
			local worms = minetest.find_nodes_in_area(posNeg, posPos, "sas_core:cave_worms")
...
function place_worms(pos)
	...
	for i, cpos in ipairs(axes) do
		if minetest.get_node(cpos).name == "default:stone" then
			...
			minetest.set_node(pos, { name = "sas_core:cave_worms", param2 = facedir })
			return
		end
	end
end
...
minetest.register_node("sas_core:stone_with_worms", {
	description = S("Glow Worms in Stone"),
	tiles = { "default_stone.png^worms.png" },
	is_ground_content = true,
	groups = { cracky = 1 },
	sounds = default.node_sound_stone_defaults(),
	drop = "sas_core:stone_with_worms",
	paramtype = "light",
	light_source = 4,
})

minetest.register_abm({
	nodenames = { "sas_core:stone_with_worms" },
	interval = 60.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		minetest.set_node(pos, { name = "sas_core:stone" })
	end,
})
...
minetest.register_node("sas_core:shrooms", {
	description = S("No-Glow Shrooms"),
	drawtype = "plantlike",
	tiles = { "shrooms.png" },
	inventory_image = "shrooms.png",
	wield_image = "shrooms.png",
	drop = 'sas_core:shrooms',
	groups = { snappy=3, flammable=2, flower=1, flora=1, attached_node=1, dig_by_water },
	sunlight_propagates = true,
	walkable = false,
	pointable = true,
	diggable = true,
...
minetest.register_abm({
    ...
    local posPos = { x=pos.x+1, y=pos.y+1, z=pos.z+1 }
		local shrooms = minetest.find_nodes_in_area(posNeg, posPos, "sas_core:shrooms")
...
function add_shrooms(pos)
	if minetest.find_node_near(pos, 2, "sas_core:shrooms") == nil then
		for nx = -1, 1, 2 do
			for ny = -1, 1, 1 do
				for nz = -1, 1, 2 do
					local tpos = { x=pos.x+nx, y=pos.y-1+ny, z=pos.z+nz }
					if minetest.get_node(tpos).name == "sas_core:dirt_with_grass" and math.random() < 0.2 then
						local ppos = { x=tpos.x, y=tpos.y+1, z=tpos.z }
						minetest.set_node(ppos, { name = "sas_core:shrooms" })
					end
				end
			end
		end
	...
	if minetest.find_node_near(pos, 2, "sas_core:shrooms") == nil then
		for nx = -1, 1, 2 do
			for ny = -1, 1, 1 do
				for nz = -1, 1, 2 do
					local tpos = { x=pos.x+nx, y=pos.y-1+ny, z=pos.z+nz }
					if minetest.get_node(tpos).name == "sas_core:dirt_with_leaves_1" and math.random() < 0.2 then
						local ppos = { x=tpos.x, y=tpos.y+1, z=tpos.z }
						minetest.set_node(ppos, { name = "sas_core:shrooms" })
					end
				end
...
minetest.register_node("sas_core:fireflies", {
	...
	climbable = false,
	buildable_to = true,
	groups = { flammable=2, dig_by_water },
})
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		if minetest.get_timeofday() > 0.74 or minetest.get_timeofday() < 0.22 then
			if minetest.find_node_near(pos, 9, "sas_core:fireflies") == nil then
				minetest.set_node(pos, {name = "sas_core:fireflies"})
			end
		end
	end,
})
