...
sas_core.modpath = minetest.get_modpath("plants_lib")
...
function sas_core:populate_surfaces(biome, nodes_or_function_or_model, snodes, checkair)
	...
	for i = 1, #snodes do
		...
		local noise2 = sas_core.perlin_temperature:get_2d({x=pos.x, y=pos.z})
		local noise3 = sas_core.perlin_humidity:get_2d({x=pos.x+150, y=pos.z+50})
...
function sas_core:spawn_on_surfaces(sd,sp,sr,sc,ss,sa)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local perlin_fertile_area = minetest.get_perlin(biome.seed_diff, perlin_octaves, perlin_persistence, perlin_scale)
			local noise1 = perlin_fertile_area:get_2d({x=p_top.x, y=p_top.z})	--fix depreciated get_2d to get_2d
			local noise2 = sas_core.perlin_temperature:get_2d({x=p_top.x, y=p_top.z})	--fix depreciated get_2d to get_2d
			local noise3 = sas_core.perlin_humidity:get_2d({x=p_top.x+150, y=p_top.z+50})	--fix depreciated get_2d to get_2d
...
minetest.after(0, function()
	print("[Plants Lib] Registered a total of "..(#sas_core.surfaceslist_aircheck)+(#sas_core.surfaceslist_no_aircheck).." surface types to be evaluated, spread")
	print("[Plants Lib] across "..#sas_core.actionslist_aircheck.." actions with air-checking and "..#sas_core.actionslist_no_aircheck.." actions without.")
end)
