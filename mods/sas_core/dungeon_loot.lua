...
sas_core.depth_first_basic_increase = 200
sas_core.depth_second_basic_increase = 2000
...
-- "sas_core.name_list" where "name" is in the above list
...
-- can grow pretty big.
sas_core.treasure_list = {
	{name="default:steel_ingot", min_depth = 0},
	{name="tnt:tnt", min_depth = 20},
	{name="sas_core:tool_sword_stone", min_depth = 45},
	{name="sas_core:chain", min_depth = 50},
	{name="sas_core:iron_bars", min_depth = 150},
	...
	{name="sas_core:iron_bloom", min_depth = 777},
...
sas_core.tools_list = {
	{name="sas_core:tool_sword_stone", min_depth = 0},
	{name="sas_core:handle_medium", min_depth = 38},
	{name="sas_core:tool_pick_iron", min_depth = 40},
	{name="sas_core:tool_axe_hardened_steel", min_depth = 95},
	{name="sas_core:tool_pick_hardened_steel", min_depth = 100}
}

sas_core.weapons_list = {
	{name="sas_core:tool_sword_steel", min_depth = 0},
	{name="sas_core:sieve0", min_depth = 50},
	{name="sas_core:chain", min_depth = 150},
	{name="sas_core:tool_sword_hardened_steel", min_depth = 250}
}
...
--   table.insert(sas_core.treasure_list, 5, {name="your_mod:platinum_ingot", min_depth = 120}
...
if minetest.get_modpath("farming") then
 	table.insert(sas_core.consumables_list, {name="farming:bread", chance_and_amount = 10})
 	table.insert(sas_core.seedlings_list, {name="farming:seed_wheat", chance = 1, amount = 10})
 	--table.insert(sas_core.seedlings_list, {name="farming:seed_cotton", chance = 20, amount = 5})
end
...
local function get_basic_loot(loot_list, depth)
	...
	if depth > sas_core.depth_first_basic_increase then
		loot_amount = math.random(1,type_amount)
	end
	if depth > sas_core.depth_second_basic_increase then
...
local function get_item_and_amount(list_item, actual_depth)
	...
	-- Suspicious trickery
	list_name = nil
	list_name_string = "sas_core." .. list_item.name .. "_list"
...
local function fill_chest(pos)
	minetest.after(2, function()
		...
		if n and n.name and n.name == "default:chest" then
			...
			for i,v in ipairs(sas_core.loot_types) do
...
core.register_on_generated(function(minp, maxp, blockseed)
	...
	if ntf and ntf.dungeon and #ntf.dungeon >= sas_core.min_num_of_rooms then
