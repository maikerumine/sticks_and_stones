...
function sas_core.can_grow(pos)
...
function sas_core.grow_sapling(pos)
	if not sas_core.can_grow(pos) then
	...
	if node.name == "sas_core:sapling" then
		minetest.log("action", "A sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		if mg_name == "v6" then
			sas_core.grow_tree(pos, random(1, 4) == 1)
		else
			sas_core.grow_new_apple_tree(pos)
		end
	elseif node.name == "sas_core:junglesapling" then
		minetest.log("action", "A jungle sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		if mg_name == "v6" then
			sas_core.grow_jungle_tree(pos)
		else
			sas_core.grow_new_jungle_tree(pos)
		end
	elseif node.name == "sas_core:pine_sapling" then
		minetest.log("action", "A pine sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		local snow = is_snow_nearby(pos)
		if mg_name == "v6" then
			sas_core.grow_pine_tree(pos, snow)
		elseif snow then
			sas_core.grow_new_snowy_pine_tree(pos)
		else
			sas_core.grow_new_pine_tree(pos)
		end
	elseif node.name == "sas_core:acacia_sapling" then
		minetest.log("action", "An acacia sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		sas_core.grow_new_acacia_tree(pos)
	elseif node.name == "sas_core:aspen_sapling" then
		minetest.log("action", "An aspen sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		sas_core.grow_new_aspen_tree(pos)
	elseif node.name == "sas_core:bush_sapling" then
		minetest.log("action", "A bush sapling grows into a bush at "..
			minetest.pos_to_string(pos))
		sas_core.grow_bush(pos)
	elseif node.name == "sas_core:acacia_bush_sapling" then
		minetest.log("action", "An acacia bush sapling grows into a bush at "..
			minetest.pos_to_string(pos))
		sas_core.grow_acacia_bush(pos)
	elseif node.name == "sas_core:emergent_jungle_sapling" then
		minetest.log("action", "An emergent jungle sapling grows into a tree at "..
			minetest.pos_to_string(pos))
		sas_core.grow_new_emergent_jungle_tree(pos)
	end
end
--[[]]
minetest.register_lbm({
	name = "sas_core:convert_saplings_to_node_timer",
	nodenames = {"sas_core:sapling", "sas_core:junglesapling",
			"sas_core:pine_sapling", "sas_core:acacia_sapling",
			"sas_core:aspen_sapling"},
	action = function(pos)
...
local function add_trunk_and_leaves(data, a, pos, tree_cid, leaves_cid,
	...
	local c_apple = minetest.get_content_id("sas_core:apple")
...
function sas_core.grow_tree(pos, is_apple_tree, bad)
	--[[
	...
	--]]
	if bad then
		error("Deprecated use of sas_core.grow_tree")
	end

	local x, y, z = pos.x, pos.y, pos.z
	local height = random(5, 7)
	local c_tree = minetest.get_content_id("sas_core:tree")
	local c_leaves = minetest.get_content_id("sas_core:leaves")
...
function sas_core.grow_jungle_tree(pos, bad)
	--[[
	...
	--]]
	if bad then
		error("Deprecated use of sas_core.grow_jungle_tree")
	end
	...
	local c_air = minetest.get_content_id("air")
	local c_ignore = minetest.get_content_id("ignore")
	local c_jungletree = minetest.get_content_id("sas_core:jungletree")
	local c_jungleleaves = minetest.get_content_id("sas_core:jungleleaves")
...
function sas_core.grow_pine_tree(pos, snow)
	local x, y, z = pos.x, pos.y, pos.z
	local maxy = y + random(9, 13) -- Trunk top

	local c_air = minetest.get_content_id("air")
	local c_ignore = minetest.get_content_id("ignore")
	local c_pine_tree = minetest.get_content_id("sas_core:pine_tree")
	local c_pine_needles  = minetest.get_content_id("sas_core:pine_needles")
	local c_snow = minetest.get_content_id("sas_core:snow")
...
function sas_core.grow_new_apple_tree(pos)
...
function sas_core.grow_new_jungle_tree(pos)
	local path = minetest.get_modpath("default") ..
...
function sas_core.grow_new_emergent_jungle_tree(pos)
	local path = minetest.get_modpath("default") ..
...
function sas_core.grow_new_pine_tree(pos)
	local path
...
function sas_core.grow_new_snowy_pine_tree(pos)
	local path
	if math.random() > 0.5 then
		path = minetest.get_modpath("default") ..
...
function sas_core.grow_new_acacia_tree(pos)
	local path = minetest.get_modpath("default") ..
...
function sas_core.grow_new_aspen_tree(pos)
	local path = minetest.get_modpath("default") ..
...
function sas_core.grow_bush(pos)
	local path = minetest.get_modpath("default") ..
		"/schematics/bush.mts"
...
function sas_core.grow_acacia_bush(pos)
	local path = minetest.get_modpath("default") ..
...
function sas_core.sapling_on_place(itemstack, placer, pointed_thing,
