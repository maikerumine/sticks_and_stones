...
sas_core = {}       --was suppressed mm

--default = setmetatable({}, { __index=function(t,k) print("Use of deprecated function:", k) return default[k] end })  turned off to remove locale depends

sas_core.blocklist_aircheck = {}
sas_core.blocklist_no_aircheck = {}

sas_core.surface_nodes_aircheck = {}
sas_core.surface_nodes_no_aircheck = {}

sas_core.surfaceslist_aircheck = {}
sas_core.surfaceslist_no_aircheck = {}

sas_core.actioncount_aircheck = {}
sas_core.actioncount_no_aircheck = {}

sas_core.actionslist_aircheck = {}
sas_core.actionslist_no_aircheck = {}

sas_core.modpath = minetest.get_modpath("sas_core")

sas_core.total_no_aircheck_calls = 0
...
sas_core.plantlife_seed_diff = 329	-- needs to be global so other mods can see it
...
sas_core.perlin_temperature = PerlinNoise(temperature_seeddiff, temperature_octaves, temperature_persistence, temperature_scale)
sas_core.perlin_humidity = PerlinNoise(humidity_seeddiff, humidity_octaves, humidity_persistence, humidity_scale)

-- Local functions

function sas_core:is_node_loaded(node_pos)
...
function sas_core:set_defaults(biome)
...
function sas_core:register_generate_plant(biomedef, nodes_or_function_or_model)
	...
	if type(nodes_or_function_or_model) == "string"
	  ...
	  and not minetest.registered_nodes[nodes_or_function_or_model] then
		sas_core:dbg("Warning: Ignored registration for undefined spawn node: "..dump(nodes_or_function_or_model))
		return
	end

	if type(nodes_or_function_or_model) == "string"
	  and not string.find(nodes_or_function_or_model, ":") then
		sas_core:dbg("Warning: Registered function call using deprecated string method: "..dump(nodes_or_function_or_model))
	end

	if biomedef.check_air == false then 
		sas_core:dbg("Register no-air-check mapgen hook: "..dump(nodes_or_function_or_model))
		sas_core.actionslist_no_aircheck[#sas_core.actionslist_no_aircheck + 1] = { biomedef, nodes_or_function_or_model }
		local s = biomedef.surface
		if type(s) == "string" then
			if s and (string.find(s, "^group:") or minetest.registered_nodes[s]) then
				if not search_table(sas_core.surfaceslist_no_aircheck, s) then
					sas_core.surfaceslist_no_aircheck[#sas_core.surfaceslist_no_aircheck + 1] = s
				end
			else
				sas_core:dbg("Warning: Ignored no-air-check registration for undefined surface node: "..dump(s))
			end
		else
		...
		sas_core:dbg("Register with-air-checking mapgen hook: "..dump(nodes_or_function_or_model))
		sas_core.actionslist_aircheck[#sas_core.actionslist_aircheck + 1] = { biomedef, nodes_or_function_or_model }
		local s = biomedef.surface
		if type(s) == "string" then
			if s and (string.find(s, "^group:") or minetest.registered_nodes[s]) then
				if not search_table(sas_core.surfaceslist_aircheck, s) then
					sas_core.surfaceslist_aircheck[#sas_core.surfaceslist_aircheck + 1] = s
				end
			else
				sas_core:dbg("Warning: Ignored with-air-checking registration for undefined surface node: "..dump(s))
			end
		else
			for i = 1, #biomedef.surface do
				local s = biomedef.surface[i]
...
function sas_core:populate_surfaces(biome, nodes_or_function_or_model, snodes, checkair)

	sas_core:set_defaults(biome)
	...
	for i = 1, #snodes do
		...
		local noise2 = sas_core.perlin_temperature:get2d({x=pos.x, y=pos.z})
		local noise3 = sas_core.perlin_humidity:get2d({x=pos.x+150, y=pos.z+50})
				...
				if not (biome.avoid_nodes and biome.avoid_radius and minetest.find_node_near(p_top, biome.avoid_radius + math.random(-1.5,2), biome.avoid_nodes)) then
					...
					if objtype == "table" then
						if nodes_or_function_or_model.axiom then
							sas_core:generate_tree(pos, nodes_or_function_or_model)
							spawned = true
						else
							local fdir = nil
					...
					elseif objtype == "string" and pcall(loadstring(("return %s(...)"):
						...
						sas_core:dbg("Warning: Ignored invalid definition for object "..dump(nodes_or_function_or_model).." that was pointed at {"..dump(pos).."}")
					end
				else
					tries = tries + 1
				end
...
function sas_core:generate_block_with_air_checking()
	if #sas_core.blocklist_aircheck > 0 then

		local minp =		sas_core.blocklist_aircheck[1][1]
		local maxp =		sas_core.blocklist_aircheck[1][2]
		...
		if not sas_core.surface_nodes_aircheck.blockhash then

			if type(minetest.find_nodes_in_area_under_air) == "function" then -- use newer API call
				sas_core.surface_nodes_aircheck.blockhash =
					minetest.find_nodes_in_area_under_air(minp, maxp, sas_core.surfaceslist_aircheck)
			else
				local search_area = minetest.find_nodes_in_area(minp, maxp, sas_core.surfaceslist_aircheck)
				...
				local pos = search_area[i]
					...
					if minetest.get_node(p_top).name == "air" then
						sas_core.surface_nodes_aircheck.blockhash[#sas_core.surface_nodes_aircheck.blockhash + 1] = pos
					end
				end
			end
			sas_core.actioncount_aircheck.blockhash = 1

		else
			if sas_core.actioncount_aircheck.blockhash <= #sas_core.actionslist_aircheck then
				-- [1] is biome, [2] is node/function/model
				sas_core:populate_surfaces(
					sas_core.actionslist_aircheck[sas_core.actioncount_aircheck.blockhash][1],
					sas_core.actionslist_aircheck[sas_core.actioncount_aircheck.blockhash][2],
					sas_core.surface_nodes_aircheck.blockhash, true)
				sas_core.actioncount_aircheck.blockhash = sas_core.actioncount_aircheck.blockhash + 1
			else
				if sas_core.surface_nodes_aircheck.blockhash then
					table.remove(sas_core.blocklist_aircheck, 1)
					sas_core.surface_nodes_aircheck.blockhash = nil
				end
			end
		end
	end
...
function sas_core:generate_block_no_aircheck()
	if #sas_core.blocklist_no_aircheck > 0 then

		local minp =		sas_core.blocklist_no_aircheck[1][1]
		local maxp =		sas_core.blocklist_no_aircheck[1][2]

		local blockhash =	minetest.hash_node_position(minp)

		if not sas_core.surface_nodes_no_aircheck.blockhash then

			-- directly read the block to be searched into the chunk cache

			sas_core.surface_nodes_no_aircheck.blockhash =
				minetest.find_nodes_in_area(minp, maxp, sas_core.surfaceslist_no_aircheck)
			sas_core.actioncount_no_aircheck.blockhash = 1

		else
			if sas_core.actioncount_no_aircheck.blockhash <= #sas_core.actionslist_no_aircheck then
				sas_core:populate_surfaces(
					sas_core.actionslist_no_aircheck[sas_core.actioncount_no_aircheck.blockhash][1],
					sas_core.actionslist_no_aircheck[sas_core.actioncount_no_aircheck.blockhash][2],
					sas_core.surface_nodes_no_aircheck.blockhash, false)
				sas_core.actioncount_no_aircheck.blockhash = sas_core.actioncount_no_aircheck.blockhash + 1
			else
				if sas_core.surface_nodes_no_aircheck.blockhash then
					table.remove(sas_core.blocklist_no_aircheck, 1)
					sas_core.surface_nodes_no_aircheck.blockhash = nil
				end
			end
		end
	end
...
minetest.register_on_generated(function(minp, maxp, blockseed)
	sas_core.blocklist_aircheck[#sas_core.blocklist_aircheck + 1] = { minp, maxp }
end)

minetest.register_on_generated(function(minp, maxp, blockseed)
	sas_core.blocklist_no_aircheck[#sas_core.blocklist_no_aircheck + 1] = { minp, maxp }
end)
...
minetest.register_globalstep(function(dtime)
	if dtime < 0.2 and    -- don't attempt to populate if lag is already too high
	  (#sas_core.blocklist_aircheck > 0 or #sas_core.blocklist_no_aircheck > 0) then
		sas_core.globalstep_start_time = minetest.get_us_time()
		sas_core.globalstep_runtime = 0
		while (#sas_core.blocklist_aircheck > 0 or #sas_core.blocklist_no_aircheck > 0)
		  and sas_core.globalstep_runtime < 200000 do  -- 0.2 seconds, in uS.
			if #sas_core.blocklist_aircheck > 0 then
				sas_core:generate_block_with_air_checking()
			end
			if #sas_core.blocklist_no_aircheck > 0 then
				sas_core:generate_block_no_aircheck()
			end
			sas_core.globalstep_runtime = minetest.get_us_time() - sas_core.globalstep_start_time
		end
	end
end)
...
minetest.register_on_shutdown(function()
	print("[default] Stand by, playing out the rest of the aircheck mapblock log")
	print("(there are "..#sas_core.blocklist_aircheck.." entries)...")
	while true do
		sas_core:generate_block_with_air_checking(0.1)
		if #sas_core.blocklist_aircheck == 0 then return end
	end
end)

minetest.register_on_shutdown(function()
	print("[default] Stand by, playing out the rest of the no-aircheck mapblock log")
	print("(there are "..#sas_core.blocklist_aircheck.." entries)...")
	while true do
		sas_core:generate_block_no_aircheck(0.1)
		if #sas_core.blocklist_no_aircheck == 0 then return end
	end
end)

-- The spawning ABM

function sas_core:spawn_on_surfaces(sd,sp,sr,sc,ss,sa)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local noise2 = sas_core.perlin_temperature:get2d({x=p_top.x, y=p_top.z})
			local noise3 = sas_core.perlin_humidity:get2d({x=p_top.x+150, y=p_top.z+50})
			if noise1 > biome.plantlife_limit 
			  and noise2 <= biome.temp_min
			  and noise2 >= biome.temp_max
			  and noise3 <= biome.humidity_min
			  and noise3 >= biome.humidity_max
			  and sas_core:is_node_loaded(p_top) then
				...
				if not (biome.avoid_nodes and biome.avoid_radius and minetest.find_node_near(p_top, biome.avoid_radius + math.random(-1.5,2), biome.avoid_nodes))
				  ...
				  and pos.y <= biome.max_elevation
				  then
					local walldir = sas_core:find_adjacent_wall(p_top, biome.verticals_list, biome.choose_random_wall)
					if biome.alt_wallnode and walldir then
						...
						if currentsurface ~= "sas_core:water_source"
						  or (currentsurface == "sas_core:water_source" and #(minetest.find_nodes_in_area({x=pos.x, y=pos.y-biome.depth_max-1, z=pos.z}, {x=pos.x, y=pos.y, z=pos.z}, {"sas_core:dirt", "sas_core:dirt_with_grass", "sas_core:sand"})) > 0 )
						  then
							...
							elseif biome.spawn_on_side then
								local onside = sas_core:find_open_side(pos)
								if onside then 
...
function sas_core:grow_plants(opts)

	local options = opts

	options.height_limit = options.height_limit or 5
	options.ground_nodes = options.ground_nodes or { "sas_core:dirt_with_grass" }
	options.grow_nodes = options.grow_nodes or { "sas_core:dirt_with_grass" }
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if options.need_wall and options.verticals_list then
				walldir = sas_core:find_adjacent_wall(p_top, options.verticals_list, options.choose_random_wall)
			end
			if (n_top.name == "air" or n_top.name == "sas_core:snow")
			  and (not options.need_wall or (options.need_wall and walldir)) then
				...
				if n_bot.name == options.dry_early_node and options.grow_plant == "junglegrass:short" then
					minetest.set_node(pos, { name = "sas_core:dry_shrub" })

				elseif options.grow_vertically and walldir then
					if sas_core:search_downward(pos, options.height_limit, options.ground_nodes) then
				...
				elseif not options.grow_result and not options.grow_function then
					...
					sas_core:replace_object(pos, options.grow_result, options.grow_function, options.facedir, options.seed_diff)
				end
			end
		end
	})
...
function sas_core:replace_object(pos, replacement, grow_function, walldir, seeddiff)
	local growtype = type(grow_function)
	if growtype == "table" then
		minetest.remove_node(pos)
		sas_core:grow_tree(pos, grow_function)
		return
...
function sas_core:find_adjacent_wall(pos, verticals, randomflag)
...
function sas_core:search_downward(pos, heightlimit, ground)
...
function sas_core:find_open_side(pos)
...
function sas_core:generate_tree(pos, nodes_or_function_or_model)
	minetest.spawn_tree(pos, nodes_or_function_or_model)
end

-- and this one's for the call used in the growing code

function sas_core:grow_tree(pos, nodes_or_function_or_model)
	minetest.spawn_tree(pos, nodes_or_function_or_model)
end
...
if minetest.get_modpath("unified_inventory") or not minetest.setting_getbool("creative_mode") then
	sas_core.expect_infinite_stacks = false
else
	sas_core.expect_infinite_stacks = true
end

-- read a field from a node's definition

function sas_core:get_nodedef_field(nodename, fieldname)
...
minetest.after(0, function()
	print("[Biome Lib] Registered a total of "..(#sas_core.surfaceslist_aircheck)+(#sas_core.surfaceslist_no_aircheck).." surface types to be evaluated, spread")
	print("[Biome Lib] across "..#sas_core.actionslist_aircheck.." actions with air-checking and "..#sas_core.actionslist_no_aircheck.." actions without.")
end)
