...
-- Licence for Code and Non-Bush leaf code is WTFPL.
  ...
  minetest.register_node("sas_core:youngtree2_bottom", {
...
for i in pairs(BushBranchCenter) do
	...
	minetest.register_node("sas_core:bushbranches"..Num, {
...
for i in pairs(BushLeafNode) do
	...
	minetest.register_node("sas_core:BushLeaves"..Num, {
...
sas_core.grow_bush = function(pos)
	...
	local bush_side_height = math.random(0,1)
		...
		if chance_of_bush_node_right> 5 then
			...
			sas_core.grow_bush_node(right_pos,3,leaf_type)
		end
		...
		if chance_of_bush_node_left> 5 then
			...
			sas_core.grow_bush_node(left_pos,1,leaf_type)
		end
		...
		if chance_of_bush_node_front> 5 then
			...
			sas_core.grow_bush_node(front_pos,2,leaf_type)
		end		
		...
		if chance_of_bush_node_back> 5 then
			...
			sas_core.grow_bush_node(back_pos,0,leaf_type)
		end
		
sas_core.grow_bush_node(pos,5,leaf_type)
end

sas_core.grow_bush_node = function(pos,dir, leaf_type)
	...
	or minetest.get_node(right_here).name == "default:junglegrass" then
		minetest.set_node(right_here, {name="sas_core:bushbranches"..bush_branch_type , param2=dir})
						--minetest.chat_send_all("leaf_type: (" .. leaf_type .. ")")
		minetest.set_node(above_right_here, {name="sas_core:BushLeaves"..leaf_type})
		local chance_of_high_leaves = math.random(1,10)
		if chance_of_high_leaves> 5 then
			local two_above_right_here = {x=pos.x, y=pos.y+3, z=pos.z}
							--minetest.chat_send_all("leaf_type: (" .. leaf_type .. ")")
			minetest.set_node(two_above_right_here, {name="sas_core:BushLeaves"..leaf_type})
		end
	end
end
...
sas_core:register_generate_plant({
 ...
 sas_core.grow_youngtree2 = function(pos)
	local height = math.random(4,5)	
	sas_core.grow_youngtree_node2(pos,height)
end

sas_core.grow_youngtree_node2 = function(pos, height)
	...
	or minetest.get_node(right_here).name == "default:junglegrass" then
		if height == 4 then
				...
				minetest.set_node(right_here, {name="sas_core:youngtree2_bottom"})
				minetest.set_node(above_right_here, {name="sas_core:youngtree2_bottom"})
				minetest.set_node(two_above_right_here, {name="sas_core:bushbranches2"  , param2=2})
				minetest.set_node(two_above_right_here_south, {name="sas_core:bushbranches2"  , param2=0})
				minetest.set_node(three_above_right_here, {name="sas_core:BushLeaves1" })
				minetest.set_node(three_above_right_here_south, {name="sas_core:BushLeaves1" })
		end
