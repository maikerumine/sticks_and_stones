...
--mt_biblebot = {}
sas_core.verses = {} --This table contains all verses.
...
--[[
function sas_core.initialize() --Set the interval in minetest.conf.
	minetest.settings:set("mt_biblebot_interval",180)
	minetest.settings:save();
	return 180
end

function sas_core.set_interval() --Read the interval from minetest.conf(set it if it doesn'st exist)
	VERSE_INTERVAL = tonumber(minetest.settings:get("mt_biblebot_interval")) or sas_core.initialize()
end
]]
function sas_core.check_params(name,func,params)
...
function sas_core.read_verses()
	...
	for line in input:lines() do
		sas_core.verses[line_number] = line
		line_number = line_number + 1
	end
	io.close(input)
end
--[[
function sas_core.display_verse(verse_number)
	local vrs = sas_core.verses[verse_number] or verse_number
	if vrs then
		minetest.chat_send_all(vrs)
	end
end

function sas_core.show_verse()
	sas_core.display_message(table.random(sas_core.verses))
end
]]
...
function sas_core.show_verse()
	local vrs = string.char(0x1b).."(c@#00ff00)"..table.random(sas_core.verses)
	sas_core.display_verse(vrs)
end
--=======

function sas_core.list_verses()
	local str = ""
	for k,v in pairs(sas_core.verses) do
		str = str .. k .. " | " .. v .. "\n"
	end
	return str
end
--[[
function sas_core.remove_verse(k)
	table.remove(sas_core.verses,k)
	sas_core.save_verses()
end

function sas_core.add_verse(t)
	table.insert(sas_core.verses,table.concat(t," ",2))
	sas_core.save_verses()
end

function sas_core.save_verses()
		local output = io.open(minetest.get_worldpath().."/mt_biblebot","w")
		for k,v in pairs(sas_core.verses) do
			output:write(v .. "\n")
		end
...
--When server starts:
--sas_core.set_interval()
sas_core.read_verses()
--[[
local TIMER = 0
...
local register_chatcommand_table = {
	...
	func = function(name,param)
		...
		if t[1] == "viewverses" then
			minetest.chat_send_player(name,sas_core.list_verses())
		elseif t[1] == "removeverse" then
			if not sas_core.check_params(
			name,
			function (params)
				if not tonumber(params[2]) or
				sas_core.verses[tonumber(params[2])] == nil then
					return false,"ERROR: No such verse."
				end
				return true
			end,
			t) then return end
			sas_core.remove_verse(t[2])
		elseif t[1] == "addverse" then
			if not t[2] then
				minetest.chat_send_player(name,"ERROR: No verse.")
			else
				sas_core.add_verse(t)
			end
		else
