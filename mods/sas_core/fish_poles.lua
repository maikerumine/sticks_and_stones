...
minetest.register_alias("default:pole_wood", "sas_core:pole_wood")
minetest.register_alias("default:pole_perfect", "sas_core:pole_perfect")
...
default_setting.poles = {}
default_setting.poles.wood = {["name"] = "wood", ["max_use"] = 30, ["desc"] = S("Fishing Pole"),["bobber_max"] = 1 }  --was 2
default_setting.poles.perfect = {["name"] = "perfect", ["max_use"] = 1500, ["desc"] = S("Perfect Fishing Pole"),["bobber_max"] = 1}  --was 5


for _,pole in pairs(default_setting.poles) do
	local bobbermax = pole["bobber_max"]
	minetest.register_tool("sas_core:pole_".. pole.name, {
		...
		on_use = function (itemstack, user, pointed_thing)
			if pointed_thing and pointed_thing.under then
				...
				if default_setting.baits[bait] == nil then return nil end

				local objs = minetest.get_objects_inside_radius(pt.under, 1)
				for m, obj in pairs(objs) do
					if obj:get_luaentity() ~= nil and string.find(obj:get_luaentity().name, "sas_core:bobber") then
						if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, S("Sorry, there is another bobber!")) end
						return nil
					end
				end
				...
				if default_setting.contest["contest"] ~= nil and default_setting.contest["contest"] == true then
					bobber_max = default_setting.contest["bobber_nb"]
				else
					bobber_max = bobbermax
				end
				--player has others bobbers?
				for m, obj in pairs(minetest.get_objects_inside_radius(pt.under, 20)) do
					if obj:get_luaentity() ~= nil and string.find(obj:get_luaentity().name, "sas_core:bobber") ~= nil then
				...
				if bobber_nb >= bobber_max then
					if default_setting.settings["message"] == true then
				...
				--if water == -3 nodes
				if nodes < 2 then
					if default_setting.settings["message"] == true then minetest.chat_send_player(player_name, S("You don't fishing in a bottle!")) end
					return nil
				end
				local new_pos = {x=pt.under.x, y=pt.under.y+(45/64), z=pt.under.z}
				local ent = minetest.add_entity({interval = 1,x=new_pos.x, y=new_pos.y, z=new_pos.z}, default_setting.baits[bait].bobber)
				if not ent then return nil end
				local luaentity = ent:get_luaentity()
				local node = minetest.get_node_or_nil(pt.under)
				--if node and node.name == "sas_core:river_water_source" then
				if node and node.name == "sas_core:water_source" then
				...
				if default_setting.settings["wear_out"] == true and not default_setting.is_creative_mode then
					return rod_wear(itemstack, user, pointed_thing, pole.max_use)
				else
					return {name="sas_core:pole_".. pole.name, count=1, wear=0, metadata=""}
				end
			end
			return nil
		end,
		...
		on_place = function(itemstack, placer, pointed_thing)
			if default_setting.settings["simple_deco_fishing_pole"] == false then return end
			local pt = pointed_thing
			...
			if string.find(pt_under_name, "water_") == nil then
				...
				minetest.set_node(pt.above, {name="sas_core:pole_".. pole.name .."_deco", param2=direction})
	...
	minetest.register_node("sas_core:pole_".. pole.name .."_deco", {
		description = pole.desc,
		inventory_image = "fishing_pole_".. pole.name ..".png",
		...
		on_dig = function(pos, node, digger)
			if digger:is_player() and digger:get_inventory() then
				...
				digger:get_inventory():add_item("main", {name="sas_core:pole_".. pole.name, count=1, wear=wear_out, metadata=""})
			end
