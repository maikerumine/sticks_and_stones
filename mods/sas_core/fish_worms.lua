...
minetest.register_alias("default:bait_worm_entity", "sas_core:bait_worm_entity")
...
-- WORM ITEM
-----------------------------------------------------------------------------------------------
minetest.register_craftitem("sas_core:bait_worm", {
	description = default_setting.func.S("Worm"),
	groups = { fishing_bait=1 },
	inventory_image = "fishing_bait_worm.png",
	on_place = function(itemstack, placer, pointed_thing)
		local pt = pointed_thing
		minetest.add_entity({x=pt.under.x, y=pt.under.y+0.6, z=pt.under.z}, "sas_core:bait_worm_entity")
		itemstack:take_item()
		return itemstack
	end,
	on_drop = function(itemstack, dropper, pos)
		minetest.add_entity({x = pos.x, y = pos.y, z = pos.z}, "sas_core:bait_worm_entity")
		itemstack:take_item()
		return itemstack
	end,
})
...
-- WORM MOB
-----------------------------------------------------------------------------------------------
minetest.register_entity("sas_core:bait_worm_entity", {
	hp_max = 300,
	damage_over_time = 1,
	...
	on_rightclick = function(self, clicker)
		if clicker:is_player() and clicker:get_inventory() then
			clicker:get_inventory():add_item("main", "sas_core:bait_worm")
			self.object:remove()
		end
	end,
	-- AI :D
	on_step = function(self, dtime)
		...
		local look_whats_up = function(self)
			...
			elseif string.find(n.name, "sas_core:water") then -- sink when in water
...
if default_setting.settings["new_worm_source"] == false then
	minetest.register_node("default:dirt", {
		description = default_setting.func.S("Dirt"),
		tiles = {"default_dirt.png"},
		is_ground_content = true,
		groups = {crumbly=3},
		sounds = sas_core.node_sound_dirt_defaults(),
		after_dig_node = function (pos, oldnode, oldmetadata, digger)
				if math.random(1, 100) <= default_setting.settings["worm_chance"] then
				local tool_in_use = digger:get_wielded_item():get_name()
				if tool_in_use == "" or tool_in_use == "default:dirt" then
					if default_setting.settings["worm_is_mob"] == true then
						minetest.add_entity({x = pos.x, y = pos.y+0.4, z = pos.z}, "sas_core:bait_worm_entity")
					else
						local inv = digger:get_inventory()
						if inv:room_for_item("main", {name="sas_core:bait_worm", count=1, wear=0, metadata=""}) then
							inv:add_item("main", {name="sas_core:bait_worm", count=1, wear=0, metadata=""})
						end
					end
				end
			end
	...
	local function hoe_on_use(itemstack, user, pointed_thing, uses)
		...
		if math.random(1, 100) < default_setting.settings["worm_chance"] then
			if default_setting.settings["worm_is_mob"] == true then
				minetest.add_entity({x=pt.under.x, y=pt.under.y+0.4, z=pt.under.z}, "sas_core:bait_worm_entity")
			else
	...
	minetest.register_tool(":farming:hoe_wood", {
		description = default_setting.func.S("Wooden Hoe"),
	...
	minetest.register_tool(":farming:hoe_stone", {
		description = default_setting.func.S("Stone Hoe"),
	...
	minetest.register_tool(":farming:hoe_steel", {
		description = default_setting.func.S("Steel Hoe"),
	...
	minetest.register_tool(":farming:hoe_bronze", {
		description = default_setting.func.S("Bronze Hoe"),
	...
	--maikerumine added mese and diamond hoes
		...
		description = default_setting.func.S("Mese Hoe"),
		...
		description = default_setting.func.S("Diamond Hoe"),
