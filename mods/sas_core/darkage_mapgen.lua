...
--wherein kind of node to replace, for example sas_core:stone
      ...
      if x == 1 then
            ...
            local down = minetest.env:get_node({x=cx,y=yy-1,z=cz}).name
			      if ( up == "default:water_source" or up == "air" ) and
				      ...
				      if minetest.env:get_node({x=cx+divlen*2,y=yy,z=cz}).name == "default:water_source" then
					      num_water_around = num_water_around + 1 end
				      if minetest.env:get_node({x=cx,y=yy,z=cz-divlen*2}).name == "sas_core:water_source" then
					      num_water_around = num_water_around + 1 end
...
minetest.register_on_generated(function(minp, maxp, seed)
  ...
  generate_ore("sas_core:desert_stone_with_iron", "sas_core:desert_stone", minp, maxp, seed+0, 1/7/7/7, 3, 5, -15, 40)
  generate_claylike("sas_core:mud", minp, maxp, seed+1, 4, 0, 2, 0)
  generate_claylike("sas_core:silt", minp, maxp, seed+2, 4, -1, 1, 1)
  generate_stratus("sas_core:chalk", 
                   {"default:stone"},
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+3, 4, 25, 8, 0, -20,  50)
  generate_stratus("sas_core:ors", 
                   {"sas_defaultcore:stone"},
                   {"default:stone","air","sas_core:water_source"}, nil,
                   minp, maxp, seed+4, 4, 25, 7, 50, -200,  500)
  generate_stratus("sas_core:shale", 
                   {"default:stone"}, 
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+5, 4, 23, 7, 50, -50,  20)
  generate_stratus("sas_core:slate", 
                   {"default:stone"}, 
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+6, 6, 23, 5, 50, -500, 0)
  generate_stratus("sas_core:schist", 
                   {"default:stone"}, 
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+7, 6, 19, 6, 50, -31000, -10)
  generate_stratus("sas_core:basalt", 
                   {"default:stone"}, 
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+8, 5, 20, 5, 20, -31000, -50)
  generate_stratus("sas_core:marble", 
                   {"default:stone"},
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+9, 4, 25, 6, 50, -31000,  -75)
  generate_stratus("sas_core:serpentine", 
                   {"default:stone"},
                   {"default:stone","air"}, nil,
                   minp, maxp, seed+10, 4, 28, 8, 50, -31000,  -350)
  generate_stratus("sas_core:gneiss", 
                   {"default:stone"}, 
                   {"default:stone","air"}, nil,
