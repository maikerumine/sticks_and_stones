...
minetest.register_alias("default:bobber_box", "sas_core:bobber_box")
...
-- bobber
minetest.register_node("sas_core:bobber_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
--  DESTROY BOBBER WHEN PUNCHING IT
	on_punch = function (self, puncher, time_from_last_punch, tool_capabilities, dir)
		...
		if default_setting.settings["message"] == true then minetest.chat_send_player(playername, default_setting.func.S("You didn't catch anything."), false) end
		if not default_setting.is_creative_mode then
			...
			if inv:room_for_item("main", {name=self.bait, count=1, wear=0, metadata=""}) then
				...
				if default_setting.settings["message"] == true then minetest.chat_send_player(playername, default_setting.func.S("The bait is still there."), false) end
			end
		end
...
-- WHEN RIGHTCLICKING THE BOBBER THE FOLLOWING HAPPENS (CLICK AT THE RIGHT TIME WHILE HOLDING A FISHING POLE)
	on_rightclick = function (self, clicker)
		...
		if string.find(item_name, "sas_core:pole_") ~= nil then
			...
			if self.prize ~= "" then
				if math.random(1, 100) <= default_setting.settings["escape_chance"] then
					if default_setting.settings["message"] == true then minetest.chat_send_player(playername, default_setting.func.S("Your fish escaped."), false) end -- fish escaped
				else
					local name = self.prize[1]..":"..self.prize[2]
					local desc = self.prize[4]
					if default_setting.settings["message"] == true then minetest.chat_send_player(playername, default_setting.func.S("You caught "..desc), false) end
		...
		elseif item_name == "sas_core:baitball" then
			if not default_setting.is_creative_mode then
				inv:remove_item("main", "sas_core:baitball")
			end
			self.baitball = 20
			--addparticle
...
-- AS SOON AS THE BOBBER IS PLACED IT WILL ACT LIKE
	on_step = function(self, dtime)
		...
		if not node or string.find(node.name, "water_source") == nil then
			if default_setting.settings["message"] == true then minetest.chat_send_player(self.owner, "Haha, Fishing is prohibited outside water!") end
			self.object:remove()
			return
		end
		...
		if math.random(1, 100) <= default_setting.settings["fish_chance"] then
			if self.water_type and self.water_type == "sea" then
				self.prize = default_setting.prizes["sea"]["little"][math.random(1,#default_setting.prizes["sea"]["little"])]
			else
				self.prize = default_setting.prizes["rivers"]["little"][math.random(1,#default_setting.prizes["rivers"]["little"])]
			end
		else
...
minetest.register_entity("sas_core:bobber_fish_entity", FISHING_BOBBER_ENTITY)
