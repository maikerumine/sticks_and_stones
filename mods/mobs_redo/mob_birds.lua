...
-- load settings
	dofile(minetest.get_modpath("mobs").."/SETTINGS.txt")
...
-- gulls
	if ENABLE_GULLS then
		...
		mobs:spawn_specific("mobs:gull", l_spawn_in, l_spawn_near_gull, 5, 20, 30, l_spawn_chance_gull, 1, 0, 31000)
		mobs:register_egg("mobs:gull", "Gull", l_egg_texture, 1)
	end

-- large birds
	if ENABLE_LARGE_BIRDS then
		mobs:register_mob("mobs:bird_lg", {
			type = "animal",
			passive = true,
			hp_min = 5,
			hp_max = 10,
		...
		--name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs:bird_lg", l_spawn_in, l_spawn_near_bird, 5, 20, 30, l_spawn_chance_bird, 1, 0, 31000)
		mobs:register_egg("mobs:bird_lg", "Large bird", l_egg_texture, 1)
	end

-- small birds
	if ENABLE_SMALL_BIRDS then
		mobs:register_mob("mobs:bird_sm", {
			type = "animal",
			passive = true,
			hp_min = 2,
			hp_max = 5,
		...
		--name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs:bird_sm", l_spawn_in, l_spawn_near_bird, 5, 20, 30, l_spawn_chance_bird, 1, 0, 31000)
		mobs:register_egg("mobs:bird_sm", "Small bird", l_egg_texture, 1)
	end
